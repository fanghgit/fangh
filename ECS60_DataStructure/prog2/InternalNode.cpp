...
void InternalNode::removeLeft(int pos)
{
    BTreeNode* tmpchildren = ((InternalNode*) leftSibling)->children[leftSibling->getCount() - 1];
    int tmpkey = ((InternalNode*) leftSibling)->keys[leftSibling->getCount() - 1];
    ((InternalNode*) leftSibling)->removechildren(leftSibling->getCount() - 1);
    for(int i = count; i > 0; i++){
        children[i] = children[i-1];
        keys[i] = keys[i-1];
    }
    count++;
    children[0] = tmpchildren;
    keys[0] = tmpkey;
    if(parent){
...
void InternalNode::removeRight(int pos)
{
    BTreeNode* tmpchildren = ((InternalNode*) rightSibling)->children[0];
    int tmpkey = ((InternalNode*) rightSibling)->keys[0];
    ((InternalNode*) rightSibling)->removechildren(0);
    count++;
    children[count - 1] = tmpchildren;
    keys[count -1] = tmpkey;
    if(parent){
...
void InternalNode::removechildren(int pos){
...
void InternalNode::removeThis(BTreeNode* ptr, int pos){
    if(ptr == children[pos - 1]){
...
{  // to be written by students
    ...
    if(leftSibling){
        return ImergeLeft(pos);
    }
    
    
    if(rightSibling && rightSibling->getCount() > (internalSize + 1)/2)
    {
        removeRight(pos);
        return NULL;
    }
...
//add
BTreeNode* InternalNode::ImergeLeft(int pos){
    BTreeNode* tmp = leftSibling;
    for(int i = (count + leftSibling->getCount() - 1); i >= leftSibling->getCount(); i--)
    {
        children[i] = children[i - leftSibling->getCount()];
        keys[i] = keys[i - leftSibling->getCount()];
    }
    for(int i = leftSibling->getCount() - 1; i >= 0; i--){
        children[i] = ((InternalNode*) leftSibling)->children[i];
    }
...
BTreeNode* InternalNode::ImergeRight(int pos){
    BTreeNode* tmp = rightSibling;
    for(int i = count; i < (count + rightSibling->getCount()); i++)
    {
        children[i] = ((InternalNode*) rightSibling)->children[i - count];
        keys[i] = ((InternalNode*) rightSibling)->keys[i - count];
    }
...
BTreeNode* InternalNode::getchildren(){
    return children[0];
}
